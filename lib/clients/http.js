...
let standardizeReqOpts = helpers.standardizeReqOpts;
...
module.exports = function (config) {
  ...
  if (config.prefix) baseUrl = baseUrl + config.prefix;
  ...
  client.request = function *(opts) {
    opts = standardizeReqOpts(opts);

    opts.url = path.join(baseUrl + opts.path);
    ...
    if (response.body) {
      ...
      result.data = ressponse.body.data || response.body;
    } else {
      result = {
